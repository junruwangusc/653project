...
hot_threshold = 2 # manually set, hyperparameter
#---------------------------------------------------------
# Cache parameters
    ...
    def replacement(self, vec_degree): # LRU
        if(vec_degree>=hot_threshold):
            if(0 in self.hot_entry_valid): # if there are void place
                ...
                earlist_t = min(self.hot_entry_visit_time) 
                loc = self.hot_entry_visit_time.index(earlist_t) #return the least recent used location
                return loc, 2            
        else: # vec_degree<hot_threshold
