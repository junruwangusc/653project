...
hot_dosa = 8
cache_depth = 1 << 13 # set field is 17 bits, cache depth = 2^17
cache_width = 64 # vector element value + vector element row num
#---------------------------------------------------------
# inputbuffer (issue queue) parameters
inputbuffer_depth = 128

class Cache_entry: # for one entry
    normal_entry_sets_valuestorage = []
    normal_entry_sets_rowindex = []
    normal_entry_sets_degree = []
    normal_entry_valid = []
    normal_entry_visit_time = []
    hot_entry_sets_valuestorage = []
    hot_entry_sets_rowindex = []
    hot_entry_sets_degree = []
    hot_entry_valid = []
    hot_entry_visit_time = []
    def __init__(self, dosa, cache_width): # create a cache line
        normal_dosa = dosa-hot_dosa
        self.normal_entry_sets_valuestorage = [0 for i in range(normal_dosa)]
        self.normal_entry_sets_rowindex = [0 for i in range(normal_dosa)]
        self.normal_entry_sets_degree = [0 for i in range(normal_dosa)]
        self.normal_entry_valid = [0 for i in range(normal_dosa)]
        self.normal_entry_visit_time = [-1 for i in range(normal_dosa)]
        self.hot_entry_sets_valuestorage = [0 for i in range(hot_dosa)]
        self.hot_entry_sets_rowindex = [0 for i in range(hot_dosa)]
        self.hot_entry_sets_degree = [0 for i in range(hot_dosa)]
        self.hot_entry_valid = [0 for i in range(hot_dosa)]
        self.hot_entry_visit_time = [-1 for i in range(hot_dosa)]        
    def check(self, vec_row):
        if (vec_row  in self.normal_entry_sets_rowindex):
            vec_index = self.normal_entry_sets_rowindex.index(vec_row) 
            if (self.normal_entry_valid[vec_index]):
                return 1 
                # if the row can be find and valid
        elif (vec_row  in self.hot_entry_sets_rowindex):
            vec_index = self.hot_entry_sets_rowindex.index(vec_row)
            if (self.hot_entry_valid[vec_index]):
                return 2
        return 0

    def replacement(self, vec_degree): # LRU
        least_deg_t = min(self.hot_entry_sets_degree)
        if(vec_degree>=least_deg_t):
            if(0 in self.hot_entry_valid): # if there are void place
                loc = self.hot_entry_valid.index(0) 
                if(0 in self.normal_entry_valid): # find the normal cache place needs to be sacrified
                    t_loc = self.normal_entry_valid.index(0) 
                else:
                    earlist_t = min(self.normal_entry_visit_time) 
                    t_loc = self.normal_entry_visit_time.index(earlist_t) 
                self.normal_entry_sets_rowindex[t_loc] = self.hot_entry_sets_rowindex[loc]
                self.normal_entry_sets_valuestorage[t_loc] = self.hot_entry_sets_valuestorage[loc]
                self.normal_entry_sets_degree[t_loc] = self.hot_entry_sets_degree[loc]
                self.normal_entry_valid[t_loc] = self.hot_entry_valid[loc]
                self.normal_entry_visit_time[t_loc] = self.hot_entry_visit_time[loc]
                return loc, 2  # return the first void place
            else:
                loc = self.hot_entry_sets_degree.index(least_deg_t) #return the least recent used location
                if(0 in self.normal_entry_valid): # find the normal cache place needs to be sacrified
                    t_loc = self.normal_entry_valid.index(0) 
                else:
                    earlist_t = min(self.normal_entry_visit_time) 
            ...
            if(0 in self.normal_entry_valid): # if there are void place
                loc = self.normal_entry_valid.index(0) 
                return loc, 1  # return the first void place
            else:
                earlist_t = min(self.normal_entry_visit_time) 
                loc = self.normal_entry_visit_time.index(earlist_t) #return the least recent used location
                return loc, 1
        print("error") # the location must be found, otherwise there is error
        return 0, 0

    def read(self, vec_row, visit_time):
        if (self.check(vec_row)==1):
            r_loc = self.normal_entry_sets_rowindex.index(vec_row) 
            value_t = self.normal_entry_sets_valuestorage[r_loc]
            self.normal_entry_visit_time[r_loc] = visit_time
            return value_t
        elif (self.check(vec_row)==2):
            r_loc = self.hot_entry_sets_rowindex.index(vec_row) 
            value_t = self.hot_entry_sets_valuestorage[r_loc]
            self.hot_entry_visit_time[r_loc] = visit_time
            return value_t            
        return 0
                      
    def write(self, vec_row, vec_value, vec_degree, visit_time): # should be used only when the value is not found in the cache
        if (self.check(vec_row)==1): # the cache has the value already
            return 0
        elif (self.check(vec_row)==2): # the hot cache has the value already
            return 0
        else: # need to be written into the cache
            w_loc, sel = self.replacement(vec_degree)
            if (sel==1):
                self.normal_entry_sets_rowindex[w_loc] = vec_row
                self.normal_entry_sets_valuestorage[w_loc] = vec_value
                self.normal_entry_sets_degree[w_loc] = vec_degree
                self.normal_entry_valid[w_loc] = 1
                self.normal_entry_visit_time[w_loc] = visit_time
            elif (sel==2):
                self.hot_entry_sets_rowindex[w_loc] = vec_row
                self.hot_entry_sets_valuestorage[w_loc] = vec_value
                self.hot_entry_sets_degree[w_loc] = vec_degree
                self.hot_entry_valid[w_loc] = 1
                self.hot_entry_visit_time[w_loc] = visit_time                
        return 0
...
class Cache: # for one entry
    ...
    def read(self, vec_row, vec_value, vec_degree, visit_time): # should be used after check
        vec_row_in_mem = -1
        entry_index = self.find_entry(vec_row)
        entry_obj = self.cache_memory[entry_index]
    ...
    def write(self, vec_row, vec_value, vec_degree, visit_time): # only when read from the memory, write is needed
        entry_index = self.find_entry(vec_row)
        entry_obj = self.cache_memory[entry_index]
        entry_obj.write(vec_row, vec_value, vec_degree, visit_time)
        return 0
    ...
    def events_run(self, time_step):
        ...
        if time_step in self.cache_scheduled_events:
            for event in self.cache_scheduled_events[time_step]:
                ...
                value_t = event[1]
                degree_t = event[2]
                operation_type = event[3]
                visit_time_t = time_step
                if(operation_type == "read"):
                    vec_row_in_mem_t = self.read( row_t, value_t, degree_t, visit_time_t)
                    if(vec_row_in_mem_t != -1): # if cache miss
                        vec_row_need_trace.append(vec_row_in_mem_t)
                elif(operation_type == "write"):
                    self.write(row_t, value_t, degree_t, visit_time_t)
    ...
    def request_handler(self, request_list, time_step):
        ...
        for request in request_list:
            vec_row = request[0]
            vec_value = request[1]
            vec_degree = request[2]
            operation_type = request[3]
            if(operation_type=="read"):
                self.cache_scheduled_events[time_step+self.read_delay].append((vec_row, vec_value, vec_degree, operation_type)) # use current timestep +delay
            elif(operation_type=="write"):
                self.cache_scheduled_events[time_step+self.write_delay].append((vec_row, vec_value, vec_degree, operation_type))
        return 0
...
#         self.outputfile = output_dir+"/"+file_name_without_extension+addition_info+"_trace.txt"
...
def readin_source(file_name):
    ...
    vector_row_degree_dict = {} # dictionary
    first_row_in_block = []
    ...
    for i in range(0,channel_num):
        ...
        for line in lines:
            ...
            vector_row_degree_dict[nums[0]] = nums[3]
    ...
    return matrix_block_data, vector_row_value_dict, vector_row_addr_dict, vector_row_degree_dict, first_row_in_block
...
def output_generated_results(file_name, row_value_addr_channel):
    ...
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    output_file_name = output_dir+"/"+"system_sim_result.txt"    
...
def from_vec_row_gen_cache_read_req(row_list, vec_row_value_dict, vec_row_degree_dict):
    cache_req_list = []
    for row in row_list:
        val_t = vec_row_value_dict[row]
        deg_t = vec_row_degree_dict[row]
        cache_req_list.append((row, val_t, deg_t, "read"))
    return cache_req_list

def from_vec_row_gen_cache_write_req(row_list, vec_row_value_dict, vec_row_degree_dict):
    cache_req_list = []
    for row in row_list:
        val_t = vec_row_value_dict[row]
        deg_t = vec_row_degree_dict[row]
        cache_req_list.append((row, val_t, deg_t, "write"))
    return cache_req_list

def System_sim():
    file_name = "roadNet-CA.txt"
    m_data, vec_row_value_dict, vec_row_addr_dict, vec_row_degree_dict, fisrt_row_of_blocks = readin_source(file_name)
    ...
    while (not_read_over):
        ...
        vec_row_cache_read_req = from_vec_row_gen_cache_read_req(vec_row_readin, vec_row_value_dict, vec_row_degree_dict)
        ...
        vec_row_cache_write_req = from_vec_row_gen_cache_write_req(vec_row_req_mem, vec_row_value_dict, vec_row_degree_dict)
